...
// material properties.
// distance, hit normal, hit material
struct Intersect {
    float len;
    vec3 normal;
    Material material;
...
// For a Static Light
// color, direction
Light light = Light(vec3(1.0) * intensity, normalize(vec3(-1.0, 0.75, 1.0*sin(iTime))));
...
Intersect intersect(Ray ray, Sphere sphere) {
    ...
    // if both positive, ray facing the sphere
    // if one positive one negative, ray shoot from inside
             float len = l - sqrt(det); // smaller
    if (len < 0.0) len = l + sqrt(det);
    if (len < 0.0) return miss; // if both negative, the sphere is behind the ray
    vec3 hit_normal = (ray.origin + len*ray.direction - sphere.position) / sphere.radius;
    return Intersect(len, hit_normal, sphere.material);
}
...
Intersect trace(Ray ray) {
    ...
    spheres[0] = Sphere(2.0, vec3(-4.0, 3.0/* + sin(iTime)*/, 0), Material(vec3(1.0, 0.0, 0.2), 1.0, 0.1));// color,diff,spec
...
void main(void) {
    ...
    gl_FragColor = vec4(radiance(ray) * exposure,1.);
}
